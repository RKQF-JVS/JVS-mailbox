<template>
  <div class="jvs-table">
    ...
    <div class="table-body-box">
      ...
      >
        ...
        <el-table-column
          ...
          :label="item.label"
          :prop="item.prop"
          :show-overflow-tooltip="item.type == 'image' ? false : option.showOverflow"
        ...
        >
          <template slot-scope="scope" >
            <span>
              ...
              <div v-if="!item.slot && !item.expand && (!item.expressControl || item.expressControl.length == 0)">
                <!-- 一般列 -->
    ...
    <el-dialog
      ...
      append-to-body
      :width="option.dialogWidth || '40%'"
      :fullscreen="option.dialogWidth == '100%' ? true : false"
      :class="{'form-fullscreen-dialog': option.dialogWidth == '100%'}"
      :before-close="handleClose">
      <jvs-form :formData="rowData" :defalutFormData="rowData" :option="formOpton" @submit="submitHandle" :isSearch="false" @cancalClick="handleClose">
      </jvs-form>
    </el-dialog>
  </div>
</template>
...
export default {
      ...
      if (this.selectedRows && this.selectedRows.length > 0 && this.$refs[this.refs]) {
        this.selectedRows.forEach(row => {
          for(let i in this.data) {
            ...
            let k2 = Object.keys(row)[0]
            if(keys.indexOf(k2) > -1) { //  && this.data[i][k2] == row[k2]
  ...
  methods: {
    ...
    styleHeight () {
      ...
      height = height - 16 - 36 - 10 - 126
      if($('.jvs-table-top') && $('.jvs-table-top').length > 0) {
        if($('.pageheader-top')) {
          height -= $('.pageheader-top').height() - 10
        }
        if($('.jvs-table-top') && $('.jvs-table-top').length > 0) {
          height -= $('.jvs-table-top').height() - 20
        }else{
          height -= 8
        }
      }else{
        if($('.jvs-table-titleTop') && $('.jvs-table-titleTop').length > 0) {
          height -= ($('.jvs-table-titleTop').height() - 18)
        }else{
          height -= 8
        }
      }
